...
public class Visualize {
    ...
    public Bitmap draw(Bitmap inputImage, Tensor outputTensor,Bitmap bg,boolean isPng){
        ...
        Bitmap mALPHA_IMAGE = floatArrayToBitmap(arralist,(int)outputShape[3],(int)outputShape[2],isPng);
    ...
    private Bitmap floatArrayToBitmap(List<Float>  floatArray,int width,int height,boolean isPng){
        byte alpha = (byte) 255 ;
        ...
        float delta = Maximum - minmum + 0.000001f ;
        ...
        for (float value : floatArray){
            ...
            byteBuffer.put(4*i+3, isPng?temValue:alpha) ;
            i++;
        }
    ...
    private Bitmap synthetizeBitmap(Bitmap front,Bitmap background, Bitmap alpha){
        ...
        for (int row=0; row < height; row++){
            for (int col=0; col < width; col++){
                ...
                frontPixels[index]=(int)alphaA<<24|((int)frontR<<16)|((int)frontG<<8)|(int)frontB;
            }
        }
